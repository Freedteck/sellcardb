...
import { ArrowLeft, Star, MapPin, MessageCircle, Phone, Mail, Globe, Instagram, Eye, Users, Share2, Calendar } from 'lucide-react';
...
const ServiceDetail: React.FC = () => {
  ...
  const [service, setService] = useState<Service & { seller: Seller } | null>(null);
  ...
  const handleContactSeller = (method: 'whatsapp' | 'phone' | 'email') => {
    ...
    switch (method) {
      case 'whatsapp':
        const whatsappUrl = `https://wa.me/${service.seller.whatsapp_number.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(message)}`;
      ...
      case 'email':
        if (service.seller.email) {
          window.open(`mailto:${service.seller.email}?subject=Inquiry about ${service.name}&body=${encodeURIComponent(message)}`, '_blank');
        }
        break;
    }
  };
  ...
  const shareService = () => {
    ...
    const shareText = service ? `Check out ${service.name} on SellCard` : 'Check out this service on SellCard';
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-4">The service you're looking for doesn't exist or is no longer available.</p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-500 dark:text-gray-400 ml-1">
                      ({service.seller.total_reviews} reviews)
                    </span>
                  </div>
            ...
            {(service.seller.website || service.seller.instagram) && (
              <div className="flex space-x-3">
                {service.seller.website && (
                  <a
                    href={service.seller.website.startsWith('http') ? service.seller.website : `https://${service.seller.website}`}
                    target="_blank"
